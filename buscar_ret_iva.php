...
<script>
	function datos( cod, det, porc, op ){
			//alert(op);
            if(op==0){
                    window.opener.document.form1.codigo_ret_iva_b.value = cod;
                    window.opener.document.form1.porc_ret_iva_b.value   = porc;
                    var base = window.opener.document.form1.base_iva_b.value;
                    var resp = ((base * porc)/100).toFixed(2);
                    window.opener.document.form1.val_iva_b.value = resp;
					//alert(resp);
            }else if(op==1){
                    window.opener.document.form1.codigo_ret_iva_s.value = cod;
                    window.opener.document.form1.porc_ret_iva_s.value = porc;
                    var base = window.opener.document.form1.base_iva_s.value;
                    var resp = ((base * porc)/100).toFixed(2);
                    window.opener.document.form1.val_iva_s.value = resp;
            }
...
//	$codigo_busca = strtr(strtoupper($codigo), "àáâãäåæçèéêëìíîïðñòóôõöøùüú", "ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÜÚ");
	$sql = "select tret_cod, tret_det_ret, tret_porct, tret_cta_cre  from saetret where
