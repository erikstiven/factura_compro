...
<script>
	function datos(op, a, b){
        if(op==1){
            window.opener.document.form1.cod_ccosn_3.value = a;
            window.opener.document.form1.ccosn_3.value     = b;
        }else if(op==2){
            window.opener.document.form1.cod_ccosn_4.value = a;
            window.opener.document.form1.ccosn_4.value     = b;
        }
...
<?	
    ...
    if ($oIfx->Query($sql)){
		...
		do {
				...
				echo '<td width="100">';		
	?>
    			<a href="#" onclick="datos(<? echo $tipo; ?>, '<? echo $cod_cuenta;?>', '<? echo $nom_cuenta;?>' ) ">
				<? echo $cont;?></a>
    <?
				echo '</td>';
				echo '<td>'
